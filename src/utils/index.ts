...
import { defaultApigenCliConfig, apigenConfigFileNames } from '../config.js';
...
/**
 * @description 存在默认的 apigen 将不会再读取配置文件
 * @param initialConfig 默认的 apigenConfig
 * @returns
 */
export async function getApigenCliConfig(initialConfig?: GeneratorConfig[]): Promise<GeneratorConfig[]> {
  const supportTs =
    fs.existsSync(path.join(process.cwd(), 'tsconfig.json')) ||
    fs.existsSync(path.join(process.cwd(), 'tsconfig.base.json'))

  const baseConfig = Object.assign({ outputExt: supportTs ? 'ts' : 'js' }, defaultApigenCliConfig)

  // 检查是否存在配置文件
  const cliConfigFileName = checkApigenConfigFile() as string
  if (!cliConfigFileName && !initialConfig) {
    throw new Error(`指定的配置文件不存在`);
  }


  let apigenConfigs = [] as GeneratorConfig[]

  try {
    apigenConfigs = initialConfig || loadFile(cliConfigFileName)
    apigenConfigs = (apigenConfigs as Record<string, any>)?.apigen || apigenConfigs

    if (!Array.isArray(apigenConfigs)) {
      throw new Error('apigenConfigs must be an array')
    }

    return apigenConfigs
  } catch (error) {
    console.error(`[apigen-cli] failed to parse configuration file, please check [${error}]`)
    return []
  }
}
 ...
 * @returns 文件路径
 */
export function checkApigenConfigFile(rootPath = process.cwd(), configFiles = apigenConfigFileNames) {
  let fileName = ''
...
export async function loadConfig(customConfigPath?: string): Promise<GeneratorConfig | GeneratorConfig[]> {
  ...
  // 优先加载编译后的 JS，再回退到 TS
  const configJsPath = path.resolve(process.cwd(), 'apigen.config.js');
  ...
  throw new Error('缺少配置文件 apigen.config.ts ');
}
